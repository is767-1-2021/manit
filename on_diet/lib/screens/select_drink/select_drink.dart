...
import 'package:on_diet/model/drink.dart';
import 'package:on_diet/screens/add_custom_drink/add_custom_drink.dart';
import 'package:on_diet/services/drink_controller.dart';
...
class SelectDrink extends StatefulWidget {
  ...
  SelectDrink(
      ...
      required this.selectedDayDrinkList});
...
class _SelectDrinkState extends State<SelectDrink> {
  ...
  Future<void> getAllDrinks() async {
    ...
    for (int i = 0; i < widget.selectedDayDrinkList.length; i++) {
      drinks.removeWhere(
          (drink) => drink.drinkId == widget.selectedDayDrinkList[i].drinkId);
    }
    setState(() {
      loading = false;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                    ...
                    child: Center(
                      child: DropdownSearch<Drink>(
                        ...
                        itemAsString: (Drink u) {
                          return u.drinkName +
                              '\n' +
                              u.totalCups.toString() +
                              "cups" +
                              " - " +
                              u.drinkKCalPerCup.toString();
                        },
                        onChanged: (data) {
                          setState(() {
                            selectedDrink = data;
                  ...
                  child: Container(
                      ...
                      child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return selectedDrinkCell(
                                widget.selectedDayDrinkList[index], index);
                          })),
                )
              ],
            )),
      bottomNavigationBar: GestureDetector(
        onTap: () async {
          await Drink.saveDrinksForDate(
              widget.selectedDate, widget.selectedDayDrinkList);
          Get.back();
        },
        child: Container(
  ...
  Widget _customDropDownExample(
      BuildContext context, Drink? drink, String itemDesignation) {
    return Container(
      child: Text(
        'Search for Drinks',
