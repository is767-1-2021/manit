...
import 'package:on_diet/screens/add_custom_food/add_custom_food.dart';
...
class SelectFood extends StatefulWidget {
  ...
  SelectFood(
      ...
      required this.selectedDayFoodList});
...
class _SelectFoodState extends State<SelectFood> {
  ...
  Future<void> getAllFoods() async {
    ...
    for (int i = 0; i < widget.selectedDayFoodList.length; i++) {
      foods.removeWhere(
          (food) => food.foodId == widget.selectedDayFoodList[i].foodId);
    }
    setState(() {
      loading = false;
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
                    ...
                    child: Center(
                      child: DropdownSearch<Food>(
                        ...
                        itemAsString: (Food u) {
                          return u.foodName +
                              '\n' +
                              u.totalDishes.toString() +
                              "Dishes" +
                              " - " +
                              u.foodKCalPerDish.toString();
                        },
                        onChanged: (data) {
                          setState(() {
                            selectedFood = data;
                  ...
                  child: Container(
                      ...
                      child: ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            return selectedFoodCell(
                                widget.selectedDayFoodList[index], index);
                          })),
                )
              ],
            )),
      bottomNavigationBar: GestureDetector(
        onTap: () async {
          await Food.saveFoodsForDate(
              widget.selectedDate, widget.selectedDayFoodList);
          Get.back();
        },
        child: Container(
  ...
  Widget _customDropDownExample(
      BuildContext context, Food? food, String itemDesignation) {
    return Container(
      child: Text(
        'Search for Foods',
